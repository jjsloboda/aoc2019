...
fn main() -> io::Result<()> {
    ...
    let signal: Vec<i64> = input.trim().chars()
        .map(|ch| ch.to_digit(10).expect("failed to parse input") as i64)
        .collect();
    ...
    // Part 2
    let mut big_signal: Vec<i64> = Vec::with_capacity(signal.len() * 10_000);
    for _ in 0..10_000 {
